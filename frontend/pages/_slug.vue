    ...
    <div class="grid">
      <div class="col-4 mt-4 pr-5">
        <div class="flex justify-content-center">
          <img class="border-circle img-height" :src="`${url}`" alt="" />
        </div>
      </div>
      ...
      <!-- Get in Touch -->
      <div
        class="col-4 inline-flex flex-column justify-content-start text-center pr-5"
      >
        ...
        <p class="font-light" v-if="linkedin">john-doe-8765893</p>
        <p class="font-semibold pt-3">Website</p>
        <p class="font-light" v-if="website">www.randomuser.dev</p>
        <p class="font-semibold pt-3">Location</p>
        <p class="font-light">T{{ city }}</p>
      </div>
      <!-- work experience sec -->
      <div class="col-8">
        ...
        <p class="font-semibold text-xl mt-1">
          {{ F_company }} | {{ F_startdate }} - {{ F_enddate }}
        </p>
        ...
        <div v-if="isJob">
          ...
          <p class="font-semibold text-xl mt-1">
            {{ S_company }} | {{ S_startdate }} - {{ S_enddate }}
          </p>
          <p class="font-light pr-7 pb-4">
            {{ S_experience }}
          </p>
      ...
      <div class="col-8">
        ...
        <p class="font-light pb-4 pr-7">
          {{ school.startdate.slice(0, 10) }} -
          {{ school.enddate.slice(0, 10) }}
        </p>
        <!-- Extra previous work Experience -->
      </div>
    </div>
...
export default {
  ...
  async created() {
    let user = this.$route.params.slug
    let param = {
      userId: user,
    }
    let res = await axios.post('http://localhost:5000/user/getdata', param)
    ...
    if (res.data.jobdetails.length > 1) {
      ...
      this.S_startdate = res.data.jobdetails[1].startdate
        .toString()
        .slice(0, 10)
      ...
      this.website = res.data.details[0].portfolio
      this.linkedin = res.data.details[0].linkedin
      this.city = res.data.details[0].city
